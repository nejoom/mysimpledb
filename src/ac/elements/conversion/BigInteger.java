...
public class BigInteger extends Number implements Comparable<BigInteger> {
    ...
    private static long bitsPerDigit[] =
            { 0, 0, 1024, 1624, 2048, 2378, 2648, 2875, 3072, 3247, 3402, 3543,
    ...
    // radix: 1112033
    // calculated n (digitsPerInt): 1
    // calculated intRadix: 1112033.0
    // 10f7e1
    ...
    public BigInteger(String val, int radix) {
        ...
        // Skip leading zeros and compute number of digits in magnitude
        while (cursor < len
                && ALong.getISOIEC10646DigitPerLong(val.codePointAt(cursor))
                        .equals("0"))
            cursor++;
        if (cursor == len) {
            signum = 0;
        ...
        if (radix < Character.MIN_RADIX || radix > Character.MAX_RADIX)
            ...
            digitsPerInt1 = digitsPerInt[radix];

        int firstGroupLen = numDigits % digitsPerInt1;
        if (firstGroupLen == 0)
            firstGroupLen = digitsPerInt1;
        String group = val.substring(cursor, cursor += firstGroupLen);
        mag[mag.length - 1] = Integer.parseInt(group, radix);
        if (mag[mag.length - 1] < 0)
            throw new NumberFormatException("Illegal digit");
        ...
        int groupVal = 0;
        while (cursor < val.length()) {
            group = val.substring(cursor, cursor += digitsPerInt1);
            groupVal = ALong.parseInt(group, radix);
            if (groupVal < 0)
                throw new NumberFormatException("Illegal digit");
            destructiveMulAdd(mag, superRadix, groupVal);
        }
    ...
    public BigInteger(int bitLength, int certainty, Random rnd) {
        ...
        prime =
                (bitLength < 95 ? smallPrime(bitLength, certainty, rnd)
    ...
    public BigInteger nextProbablePrime() {
        ...
        while (true) {
            ...
            BigInteger candidate =
                    searchSieve.retrieve(result, DEFAULT_PRIME_CERTAINTY);
            if (candidate != null)
                return candidate;
    ...
    private static int[] add(int[] x, int[] y) {
        ...
        while (yIndex > 0) {
            sum =
                    (x[--xIndex] & LONG_MASK) + (y[--yIndex] & LONG_MASK)
                            + (sum >>> 32);
    ...
    public BigInteger multiply(BigInteger val) {
        ...
        int[] result =
                multiplyToLen(mag, mag.length, val.mag, val.mag.length, null);
    ...
    public BigInteger divide(BigInteger val) {
        MutableBigInteger q = new MutableBigInteger(), r =
                new MutableBigInteger(), a = new MutableBigInteger(this.mag), b =
                new MutableBigInteger(val.mag);
    ...
    public BigInteger pow(int exponent) {
        ...
        while (exponent != 0) {
            if ((exponent & 1) == 1) {
                result =
                        multiplyToLen(result, result.length, baseToPow2,
    ...
    public BigInteger modPow(BigInteger exponent, BigInteger m) {
        ...
        BigInteger base =
                (this.signum < 0 || this.compareTo(m) >= 0 ? this.mod(m) : this);
        BigInteger result;
        if (m.testBit(0)) { // odd modulus
            ...
            BigInteger base2 =
                    (this.signum < 0 || this.compareTo(m1) >= 0 ? this.mod(m1)
                            : this);

            // Caculate (base ** exponent) mod m1.
            BigInteger a1 =
                    (m1.equals(ONE) ? ZERO : base2.oddModPow(exponent, m1));
            ...
            result =
                    a1.multiply(m2).multiply(y1).add(
    ...
    static int mulAdd(int[] out, int[] in, int offset, int len, int k) {
        ...
        for (int j = len - 1; j >= 0; j--) {
            long product =
                    (in[j] & LONG_MASK) * kLong + (out[offset] & LONG_MASK)
                            + carry;
    ...
    public BigInteger and(BigInteger val) {
        ...
        for (int i = 0; i < result.length; i++)
            result[i] =
                    (int) (getInt(result.length - i - 1) & val
    ...
    static int bitLen(int w) {
        ...
        return (w < 1 << 15 ? (w < 1 << 7 ? (w < 1 << 3 ? (w < 1 << 1
                ? (w < 1 << 0 ? (w < 0 ? 32 : 0) : 1) : (w < 1 << 2 ? 2 : 3))
                : (w < 1 << 5 ? (w < 1 << 4 ? 4 : 5) : (w < 1 << 6 ? 6 : 7)))
                : (w < 1 << 11 ? (w < 1 << 9 ? (w < 1 << 8 ? 8 : 9)
                        : (w < 1 << 10 ? 10 : 11)) : (w < 1 << 13
                        ? (w < 1 << 12 ? 12 : 13) : (w < 1 << 14 ? 14 : 15))))
                : (w < 1 << 23 ? (w < 1 << 19 ? (w < 1 << 17 ? (w < 1 << 16
                        ? 16 : 17) : (w < 1 << 18 ? 18 : 19)) : (w < 1 << 21
                        ? (w < 1 << 20 ? 20 : 21) : (w < 1 << 22 ? 22 : 23)))
                        : (w < 1 << 27 ? (w < 1 << 25 ? (w < 1 << 24 ? 24 : 25)
                                : (w < 1 << 26 ? 26 : 27)) : (w < 1 << 29
                                ? (w < 1 << 28 ? 28 : 29) : (w < 1 << 30 ? 30
                                        : 31)))));
    }
    ...
    final static byte trailingZeroTable[] =
            { -25, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0, 4, 0, 1, 0, 2,
    ...
    public String toString(int radix) {
        ...
        while (tmp.signum != 0) {
            ...
            MutableBigInteger q = new MutableBigInteger(), r =
                    new MutableBigInteger(), a = new MutableBigInteger(tmp.mag), b =
    ...
    private static int digitsPerLong[] =
            { 0, 0, 62, 39, 31, 27, 24, 22, 20, 19, 18, 18, 17, 17, 16, 16, 15,
                    15, 15, 14, 14, 14, 14, 13, 13, 13, 13, 13, 13, 12, 12, 12,
                    12, 12, 12, 12, 12 };

    private static BigInteger longRadix[] =
            { valueOf(0), valueOf(0), valueOf(0x4000000000000000L),
...
//            if (((codePoint >= 0x30) && (codePoint <= 0xD7FF))
//                    || ((codePoint >= 0xE000) && (codePoint <= 0xFFFD))
//                    || ((codePoint >= 0x10000) && (codePoint <= 0x10FFFF))) {
//                System.out.println("i: " + i + ", Character.toChars(0x"
//                        + Integer.toHexString(codePoint).toUpperCase() + "), ");
//                i++;
//            }
//        }
        i = 96;
        System.out.println("unicode has radix of: " + i);
        System.out.println(Character.digit('0',10));
        System.out.println(Character.digit('0',16));

        for (int radix = i - 5; radix <= i /* longRadix.length */; radix++) {
            try {
                System.out.println("i: " + radix + ", "
                        + longRadix[radix].doubleValue());
                System.out.println("i: " + radix + ", digitsPerLong: "
                        + digitsPerLong[radix]);
                System.out.println("i: " + radix + ", i**digistPerLong[i]: "
                        + Math.pow(radix, digitsPerLong[radix]));
                System.out.println(longRadix[radix].toString(16));
            } catch (Exception e) {
                System.out.println("unknown territory l");
            }
            // if (longRadix[radix] != null) {
            System.out.println(Math.pow(2d, 63d));
            int n = 0;
            while (Math.pow(radix, n) < Math.pow(2d, 63d)) {
                n++;
            }
            n--;
            System.out.println("radix: " + radix);
            System.out.println("calculated n (digitsPerLong): " + n);
            System.out.println("calculated longRadix: " + Math.pow(radix, n));
            System.out.println(BigInteger.valueOf((long) Math.pow(radix, n))
                    .toString(16));
            // }
        }
        System.out.println("----- " + i);
        for (int radix = /* 3 */i - 5; radix <= i /* longRadix.length */; radix++) {
            try {
                System.out.println("i: " + radix + ", " + intRadix[radix]);
                System.out.println("i: " + radix + ", digitsPerInt: "
                        + digitsPerInt[radix]);
            ...
            // if (longRadix[radix] != null) {
            System.out.println("hi: " + Math.pow(2d, 31d));
            int n = 0;
            while (Math.pow(radix, n) < Math.pow(2d, 31d)) {
                n++;
            }
            n--;
            System.out.println("radix: " + radix);
            System.out.println("calculated n (digitsPerInt): " + n);
            System.out.println("calculated intRadix: " + Math.pow(radix, n));
            System.out.println(BigInteger.valueOf((int) Math.pow(radix, n))
                    .toString(16));
            // }
        }
    }
    ...
    /**
     * Returns the String representation of this BigInteger in a radix
     * containing all ISO/IEC 10646 characters. see
     * http://www.w3.org/TR/2000/REC-xml-20001006#NT-Char. The
     ...
     * 
     * @return String representation of this BigInteger in ISO/IEC 10646
     *         character.
     * @see Integer#toString
     * @see Character#forDigit
     * @see #BigInteger(java.lang.String, int)
    ...
    public String toISOIEC10646String() {
        if (signum == 0)
            return "0";
        ...
        int numGroups = 0;
        while (tmp.signum != 0) {
            BigInteger d = valueOf(0x13158c2477d87400L);// longRadix[radix];
            ...
            digitGroup[numGroups++] = ALong.toISOIEC10646String(r2.longValue());
            tmp = q2;
        }

        // Put sign (if any) and first digit group into result buffer
        StringBuilder buf = new StringBuilder(numGroups * 3/*
                                                            * digitsPerLong[radix
                                                            * ]
                                                            */+ 1);
        if (signum < 0)
        ...
        for (int i = numGroups - 2; i >= 0; i--) {
            // Prepend (any) leading zeros for this digit group
            int numLeadingZeros =
                    3/* digitsPerLong[radix] */- digitGroup[i].length();
            if (numLeadingZeros != 0)
                buf.append(zeros[numLeadingZeros]);
            buf.append(digitGroup[i]);
        }
    ...
    private int getInt(int n) {
        ...
        return (int) (signum >= 0 ? magInt : (n <= firstNonzeroIntNum()
                ? -magInt : ~magInt));
    }
    ...
    private byte[] magSerializedForm() {
        int bitLen =
                (mag.length == 0 ? 0 : ((mag.length - 1) << 5) + bitLen(mag[0]));
        int byteLen = (bitLen + 7) / 8;
        byte[] result = new byte[byteLen];

        for (int i = byteLen - 1, bytesCopied = 4, intIndex = mag.length - 1, nextInt =
                0; i >= 0; i--) {
            if (bytesCopied == 4) {
